...
# --- Argument Parsing ---
parser = argparse.ArgumentParser(description='Train Random Forest model for time series forecasting.')
...
# Random Forest parameters (with defaults)
...
parser.add_argument('--min_samples_leaf', type=int, default=1, help='Minimum samples required at leaf node')
parser.add_argument('--max_features', type=str, default='sqrt', help='Number of features to consider for best split')
parser.add_argument('--bootstrap', type=bool, default=True, help='Whether bootstrap samples are used')
...
try:
    ...
    print(f"Building Random Forest model...")
    
    # Handle max_depth parameter
    max_depth = args.max_depth if args.max_depth != 'None' and args.max_depth is not None else None
    ...
    if args.output_dir and args.model_id:
        ...
        model_path = os.path.join(args.output_dir, f'random_forest_model_{args.model_id}.joblib')
    ...
    # --- Output Results ---
    final_message = f"Random Forest training completed successfully. Test RMSE: {test_rmse:.4f}, Test MAE: {test_mae:.4f}, Test R²: {test_r2:.4f}"
