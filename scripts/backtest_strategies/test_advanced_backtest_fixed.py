#!/usr/bin/env python3
"""
Test script cho Advanced Backtesting - Đã sửa lỗi
Sử dụng sample data để demo các tính năng với kết quả realistic
"""
...
def run_simple_walk_forward_analysis(data: pd.DataFrame) -> Dict[str, Any]:
    ...
    train_points = 2000  # ~3 months of hourly data
    test_points = 500    # ~3 weeks of hourly data
    step_points = 250    # ~1 week step
    
    periods = []
    current_start = 0
    
    while current_start + train_points + test_points <= total_points:
        train_end = current_start + train_points
    ...
    if abs(avg_test_return) > 0.01:  # If average return is significant
        consistency_score = max(0, 1 - (std_test_return / abs(avg_test_return)))
    else:
        consistency_score = 0.5  # Neutral score if returns are very small
    ...
    return {
        ...
        'summary': {
            ...
            'std_out_of_sample_return': std_test_return,
...
def main():
    ...
    print("Starting Advanced Backtesting Demo (Fixed Version)...")
    print("=" * 60)
    ...
    risk_score = max(0, 1 - (abs(mc_metrics['var']) / 20))  # Normalize VaR to 0-1
    performance_score = min(max(performance['sharpe_ratio'] / 2, 0), 1)  # Normalize Sharpe to 0-1
    ...
    if performance['total_return'] > 10:
        print(f"Total Return: EXCELLENT ({performance['total_return']:.1f}%)")
    elif performance['total_return'] > 5:
        print(f"Total Return: GOOD ({performance['total_return']:.1f}%)")
    elif performance['total_return'] > 0:
        print(f"Total Return: POOR ({performance['total_return']:.1f}%)")
    else:
        print(f"Total Return: LOSS ({performance['total_return']:.1f}%)")
    
    if performance['sharpe_ratio'] > 1.5:
        print(f"Sharpe Ratio: EXCELLENT ({performance['sharpe_ratio']:.2f})")
    elif performance['sharpe_ratio'] > 1.0:
        print(f"Sharpe Ratio: GOOD ({performance['sharpe_ratio']:.2f})")
    elif performance['sharpe_ratio'] > 0.5:
        print(f"Sharpe Ratio: POOR ({performance['sharpe_ratio']:.2f})")
    else:
        print(f"Sharpe Ratio: VERY POOR ({performance['sharpe_ratio']:.2f})")
    
    if abs(mc_metrics['var']) < 10:
        print(f"Risk (VaR): LOW ({mc_metrics['var']:.1f}%)")
    elif abs(mc_metrics['var']) < 15:
        print(f"Risk (VaR): MEDIUM ({mc_metrics['var']:.1f}%)")
    else:
        print(f"Risk (VaR): HIGH ({mc_metrics['var']:.1f}%)")
    ...
    results_file = f'results/backtests/demo/advanced_demo_fixed_{timestamp}.json'
