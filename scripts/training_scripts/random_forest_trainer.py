#!/usr/bin/env python3
"""
Random Forest Training Script - Urus Platform
Auto-generated training script for Random Forest models
"""
...
class RandomForestTrainer:
    def __init__(self, config):
        ...
        print(f"🌳 [RF Trainer] Khởi tạo Random Forest trainer: {config.get('model_name', 'Unknown')}")

    def load_data_from_supabase(self):
        """Load data từ Supabase"""
        try:
            print("📊 [RF Trainer] Đang kết nối Supabase...")
            ...
            print(f"✅ [RF Trainer] Loaded {len(data)} rows, {len(data.columns)} columns")
            
            conn.close()
            return data
            
        except Exception as e:
            print(f"❌ [RF Trainer] Error loading data: {str(e)}")
            raise

    def prepare_data(self, data):
        """Chuẩn bị data cho Random Forest"""
        try:
            print("🔧 [RF Trainer] Preparing data...")
            ...
            print(f"🎯 [RF Trainer] Task type: {task_type}")
            print(f"📊 [RF Trainer] Features: {X.shape[1]}, Target unique values: {len(np.unique(y))}")
            
            return X.values, y, task_type
            
        except Exception as e:
            print(f"❌ [RF Trainer] Error preparing data: {str(e)}")
            raise

    def build_model(self, task_type):
        """Xây dựng Random Forest model"""
        try:
            print(f"🏗️ [RF Trainer] Building Random Forest model for {task_type}...")
            ...
            print(f"✅ [RF Trainer] Model created with {params.get('n_estimators', 100)} trees")
            return self.model
            
        except Exception as e:
            print(f"❌ [RF Trainer] Error building model: {str(e)}")
            raise

    def train_model(self, X, y, task_type):
        """Train Random Forest model"""
        try:
            print("🎯 [RF Trainer] Starting training...")
            ...
            print(f"📊 [RF Trainer] Train: {len(X_train)}, Test: {len(X_test)}")
            ...
            # Train model
            print("🚀 [RF Trainer] Training Random Forest...")
            ...
            print("✅ [RF Trainer] Training completed!")
            if task_type == 'regression':
                print(f"📊 [RF Trainer] Test RMSE: {test_metrics['rmse']:.4f}")
                print(f"📊 [RF Trainer] Test R²: {test_metrics['r2']:.4f}")
            else:
                print(f"📊 [RF Trainer] Test Accuracy: {test_metrics['accuracy']:.4f}")
            
            return self.results
            
        except Exception as e:
            print(f"❌ [RF Trainer] Error training model: {str(e)}")
            raise
    ...
    def save_model_artifacts(self):
        ...
        try:
            print("💾 [RF Trainer] Saving model artifacts...")
            ...
            print(f"✅ [RF Trainer] Model saved: {model_path}")
            
            # Save scaler if used
            if hasattr(self, 'scaler') and self.scaler is not None:
                scaler_path = f"{model_dir}/scaler.pkl"
                joblib.dump(self.scaler, scaler_path)
                print(f"✅ [RF Trainer] Scaler saved: {scaler_path}")
            
            # Save label encoder if used
            if self.label_encoder is not None:
                encoder_path = f"{model_dir}/label_encoder.pkl"
                joblib.dump(self.label_encoder, encoder_path)
                print(f"✅ [RF Trainer] Label encoder saved: {encoder_path}")
            
            # Save results
            results_path = f"{model_dir}/results.json"
            with open(results_path, 'w') as f:
                json.dump(self.results, f, indent=2)
            print(f"✅ [RF Trainer] Results saved: {results_path}")
            ...
            print(f"❌ [RF Trainer] Error saving artifacts: {str(e)}")
            raise
...
def main():
    ...
    try:
        ...
        print("🌳 [RF Trainer] Starting Random Forest training...")
        ...
        print("🎉 [RF Trainer] Training thành công!")
        ...
        print(f"❌ [RF Trainer] Training failed: {str(e)}")
